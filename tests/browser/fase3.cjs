...
(async()=>{
  ...
  try{
    ...
    const cimento=page.locator('.rel-check[aria-label*="Cimento"]');
    assert.equal(await cimento.count(),2);
    ...
    await p.waitForFunction(async()=>!!(await caches.match('./vendor/sentry/sentry.js')));
    ...
    assert.ok(await p.evaluate(()=>window.CUSTTA_MONITOR && window.CusttaSentrySDK));
    assert.deepEqual(await p.evaluate(()=>__violacoes),[]);
